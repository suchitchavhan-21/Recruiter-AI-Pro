...
export default function HomeDashboard({ 
  ...
  const totalSessions = sessionsHistory?.length || 0;
  const avgScore = totalSessions > 0 
    ? Math.round(sessionsHistory.reduce((acc, s) => acc + (s.score || 0), 0) / totalSessions) 
    : null;
  ...
  return (
    <div className="relative min-h-[85vh] flex flex-col justify-between items-center py-10 px-4 md:px-8 overflow-hidden rounded-[24px] border border-white/10 bg-[#09090B]/60 backdrop-blur-2xl shadow-[0_16px_40px_0_rgba(0,0,0,0.5)] animate-fade-in">
      ...
      <div className="relative z-10 flex items-center gap-2 glass-pill px-4 py-1.5 rounded-full text-[10px] font-mono tracking-wider text-slate-200 shadow-md">
        ...
        <span>RECRUITER AI • EVIDENCE-BASED INTERVIEW & CAREER PREPARATION</span>
      </div>

      {/* Hero & Modern Tech Typography */}
      <div className="relative z-10 max-w-4xl text-center space-y-6 pt-10 animate-fade-in">
        ...
        <p className="text-[#a5b4fc] font-bold text-xs sm:text-sm tracking-widest font-mono uppercase">
          ✦ AI-Powered Interview Practice & Career Preparation ✦
        </p>
        
        <p className="text-slate-300 text-xs sm:text-sm md:text-base leading-relaxed max-w-3xl mx-auto font-medium">
          Step into realistic virtual practice sessions where specialized HR, Technical, and Hiring Manager AI agents evaluate your responses, provide structured feedback on your answers, and help you ground your interview stories in genuine technical evidence.
        </p>
        ...
        <div className="flex flex-wrap justify-center items-center gap-3.5 pt-4">
          <button
            ...
            className="px-7 py-3.5 bg-gradient-to-r from-[#6D5EF8] to-indigo-600 hover:from-[#5b4be0] hover:to-indigo-700 text-white rounded-xl text-xs sm:text-sm font-bold transition-all shadow-[0_8px_25px_0_rgba(109,94,248,0.35)] flex items-center justify-center gap-2.5 cursor-pointer border border-white/20 backdrop-blur-md group active:scale-[0.98]"
          ...
          <button
            ...
            className="px-5 py-3 glass-card hover:bg-white/10 text-slate-200 hover:text-white rounded-xl text-xs font-semibold transition-all cursor-pointer flex items-center justify-center gap-2 active:scale-[0.98]"
          >
          ...
          <button
            ...
            className="px-5 py-3 glass-card hover:bg-white/10 text-slate-300 hover:text-white rounded-xl text-xs font-semibold transition-all cursor-pointer flex items-center justify-center gap-2 active:scale-[0.98]"
          ...
          >
            ...
            <span>Voice Tuner</span>
          </button>
        </div>
      </div>

      {/* Bento Spotlight Widgets with Glassmorphism */}
      <div className="relative z-10 w-full grid grid-cols-1 md:grid-cols-3 gap-6 mt-14 max-w-5xl">
        ...
        <div className="glass-card glass-card-hover p-5 rounded-2xl flex flex-col justify-between group">
          <div className="space-y-2">
            ...
            <h3 className="text-xs font-bold text-white">Speech & Delivery Feedback</h3>
            <p className="text-[11px] text-slate-300 leading-relaxed">
              Analyzes speaking pace, pauses, and clarity during practice sessions to provide constructive suggestions on communication delivery.
            </p>
          </div>
          <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between text-[10px] font-mono text-slate-400">
            <span>Speech Feedback</span>
            <span className="text-emerald-400 font-semibold">● Audio & Text Ready</span>
          </div>
        </div>
        ...
        <div className="glass-card glass-card-hover p-5 rounded-2xl flex flex-col justify-between group">
          <div className="space-y-2">
            ...
            <h3 className="text-xs font-bold text-white">Curated & Custom JDs</h3>
            <p className="text-[11px] text-slate-300 leading-relaxed">
              Choose from industry-standard practice job presets or paste your own job description to tailor mock interview questions.
            </p>
          </div>
          <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between text-[10px] font-mono text-slate-400">
            <span>Curated Tracks</span>
            <span className="text-slate-200">Engineering • Product • Systems</span>
          </div>
        </div>
        ...
        <div className="glass-card glass-card-hover p-5 rounded-2xl flex flex-col justify-between group">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-sky-400 font-mono tracking-wider uppercase">Performance</span>
              <TrendingUp className="h-4 w-4 text-sky-400" />
            </div>
            <h3 className="text-xs font-bold text-white">Session Analytics</h3>
            <p className="text-[11px] text-slate-300 leading-relaxed">
              Tracks your evaluation scores across practice interviews and highlights strengths and improvement areas over time.
            </p>
          </div>
          <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between text-[10px] font-mono text-slate-400">
            <span>{totalSessions > 0 ? "Practice History" : "Status"}</span>
            <span className="text-sky-400 font-semibold">
              {avgScore !== null 
                ? `${avgScore}% Avg. (${totalSessions} ${totalSessions === 1 ? "session" : "sessions"})`
      ...
      <div className="relative z-10 w-full border-t border-slate-800/50 pt-8 mt-12">
        <p className="text-center text-[9px] font-mono tracking-wider text-slate-500 uppercase font-semibold mb-4">
          Practice scenarios inspired by common interview patterns (Non-affiliated practice presets)
        </p>
